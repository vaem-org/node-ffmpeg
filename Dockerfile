...
RUN         apk add --no-cache --update libgcc libstdc++ ca-certificates libcrypto3 libssl3 libgomp expat git
...
RUN \
        ...
        ./autogen.sh && \
        CXXFLAGS=-Wno-error ./configure --prefix="${PREFIX}" && \
        make && \
#        make check && \
        make install && \
        rm -rf ${DIR}
...
RUN  \
        ...
        --enable-small \
        --enable-version3 \
        --extra-cflags="-I${PREFIX}/include -Wno-error" \
